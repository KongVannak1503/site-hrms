...
import { deleteDayKpiApi, duplicateDayKpiApi, getAllDayKpiApi } from '../../../services/KpiApi';
...
const KpiDayPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await getAllDayKpiApi();
    ...
    const breadcrumbItems = [
        ...
        { breadcrumbName: content['kpiDay'] }
    ];

    useEffect(() => {
        document.title = content['kpiDay'];
    ...
    const handleDelete = async (id) => {
        try {
            await deleteDayKpiApi(id); // call the API
    ...
    const handleDuplicate = async (id) => {
        try {
            const response = await duplicateDayKpiApi(id);
            if (response?.data) {
    ...
    const handleAddCreated = () => {
        navigate('/appraisal/kpi/day/form');
    };
    const handleUpdate = (id) => {
        navigate(`/appraisal/kpi/day/update/${id}`);
    };
    ...
    return (
        <div>
            <div className="mb-3 flex justify-between">
                <p className='text-default font-extrabold text-xl'><FileTextOutlined className='mr-2' />{content['kpiDay']}</p>
