...
import { getDayKpiApi, updateDayKpiApi } from '../../../services/KpiApi';
import KpiDayForm from './KpiDayForm';

function KpiTemplateBuilderDayEditPage() {
    const { id } = useParams();
    ...
    useEffect(() => {
        async function fetchKpi() {
            try {
                const res = await getDayKpiApi(id);
    ...
    useEffect(() => {
        if (location.state?.successMessage) {
            // Delay message so it doesn't get missed during mount
            setTimeout(() => {
                message.success(location.state.successMessage);
                // Remove state so message won't show again if user refreshes
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await updateDayKpiApi(id, payload);
    ...
    const breadcrumbItems = [
        ...
        { breadcrumbName: content['kpiDay'], path: '/appraisal/kpi/day' },
...
export default KpiTemplateBuilderDayEditPage;
