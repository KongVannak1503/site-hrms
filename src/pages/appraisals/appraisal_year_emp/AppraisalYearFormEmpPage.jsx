...
import { createKpiIndividualYearEmpApi, getKpiApi, getKpiIndividualYearEmpApi, updateKpiIndividualYearEmpApi } from '../../../services/KpiApi';

const { Title } = Typography;

export default function AppraisalYearFormEmpPage() {
    ...
    useEffect(() => {
        document.title = content['appraisalYear']
        fetchTemplates();
    }, [content]);
    ...
    const fetchTemplates = async () => {
        ...
        try {
            ...
            const resInd = await getKpiIndividualYearEmpApi(mainId, id);
            if (resInd) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (!isExisting) {
                await createKpiIndividualYearEmpApi(payload);
    ...
    const breadcrumbItems = [
        ...
        { breadcrumbName: content['appraisalYear'], path: '/appraisal/year' },
    ...
    return (
        <div>
            <div className="flex justify-between mb-6">
                <h1 className='text-xl font-extrabold text-[#002060]'><FileTextOutlined className='mr-2' />{content['appraisalYear']}</h1>
            ...
            >
                ...
                <Form form={form} key={isExisting ? 'existing' : 'new'} layout="vertical" onFinish={handleSubmit}>
                    ...
                    <Card className='shadow' title={content['feedback']} style={{ marginBottom: 16 }}>
                        <Form.Item name="feedback">
                            <Input.TextArea rows={6} disabled={submitting || isExisting} />
                        </Form.Item>
                        <div className="text-end">
                            <button
                                className={`${Styles.btnCreate} disabled`}
                                type="submit"
                                disabled={!template}
                            >
                                {content['save']}
